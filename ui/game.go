...
func init() {
	Uigame.button1 = &Button{
		Rect: image.Rect(16, 16, 144, 48),
		Text: "Button 1",
	}
	Uigame.button2 = &Button{
		Rect: image.Rect(160, 16, 288, 48),
		Text: "Button 2",
	}
	Uigame.checkBox = &CheckBox{
		X:    16,
		Y:    64,
		Text: "Check Box!",
	}
	Uigame.textBoxLog = &TextBox{
		Rect: image.Rect(16, 96, 624, 464),
	}

	Uigame.button1.SetOnPressed(func(b *Button) {
		Uigame.textBoxLog.AppendLine("Button 1 Pressed")
	})
	Uigame.button2.SetOnPressed(func(b *Button) {
		Uigame.textBoxLog.AppendLine("Button 2 Pressed")
	})
	Uigame.checkBox.SetOnCheckChanged(func(c *CheckBox) {
		...
		Uigame.textBoxLog.AppendLine(msg)
	})
}
...
func (g *Game) Update() error {
	...
	//if len(touches) > 0 && firt == 0 {
	//}
	for _, t := range touches {
		x, y := ebiten.TouchPosition(t)
		g.textBoxLog.AppendLine("x：" + strconv.Itoa(x) + "** y：" + strconv.Itoa(y))

		if x <= 300 && firt == 0 {
			fmt.Println("i'm in" + strconv.Itoa(firt))
			wakeup()
			g.textBoxLog.AppendLine("upd send")
			firt++
		}
...
func GetMp() []byte {
	mac := flag.String("mac", "00:E0:B4:1C:34:E6", "help")
	flag.Parse()
	if len(*mac) == 0 {
		fmt.Printf("%s\n", "help")
		return nil
...
func wakeup() {
	...
	fmt.Println("Resolved address is ", addr.String()+":9")

	//conn, err := net.Dial("udp", addr.String()+":9")
	target := net.UDPAddr{
		IP:   addr.IP,
		Port: 9,
	}
